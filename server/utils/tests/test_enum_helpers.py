...
class EnumHelpersTestCase(unittest.TestCase):
    ...
    def test_enum_to_choices(self):
        ...
        self.assertEqual(choices[0], (self.TestEnum.key1, self.TestEnum.key1.value))
        self.assertEqual(choices[1], (self.TestEnum.key2, self.TestEnum.key2.value))

    def test_enum_to_key_list(self):
        key_list = enum_to_key_list(self.TestEnum)
        self.assertEqual(key_list, [self.TestEnum.key1, self.TestEnum.key2])

    def test_random_enum_key(self):
        with mock.patch('utils.enum_helpers.random.choice', lambda choices: choices[0]):
            self.assertEqual(random_enum_key(self.TestEnum), self.TestEnum.key1)
